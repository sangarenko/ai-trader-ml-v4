$ cat /opt/ai-trader/src/strategies/ml_predict.ts
--- rc=0 ---
/**
...
interface TreeNode {
  nodeid: number
  depth?: number
  split?: number | string  // feature index (XGBoost 3.x: "f21")
...
function walkTree(node: TreeNode, features: number[]): number {
  ...
  let featureIdx: number
  if (typeof node.split === 'string') {
    featureIdx = parseInt(node.split.replace(/^f/, ''), 10)
  } else {
    featureIdx = node.split!
  }
