...
def vectorized_backtest(ind: Dict[str, np.ndarray], closes: np.ndarray,
    ...
    elif strategy == "mean_reversion_bb":
        # V2-STRATEGIES-PATCH v1.0
        # Mean reversion with Bollinger Bands + tunable RSI thresholds.
        # See backtest/strategies_v2.py::strategy_mean_reversion_bb.
        try:
            from strategies_v2 import strategy_mean_reversion_bb
        except ImportError:
            import os, sys as _sys
            _sys.path.insert(0, os.path.dirname(os.path.abspath(__file__)))
            from strategies_v2 import strategy_mean_reversion_bb
        entry_long, entry_short, exit_long, exit_short = strategy_mean_reversion_bb(
            ind, closes, highs, lows, params, n
        )
    elif strategy == "trend_follow_adx":
        # V2-STRATEGIES-PATCH v1.0
        # Trend-follow with ADX regime filter + ATR-scaled TP.
        # Strategy writes back params['take_profit_pct'] so _backtest_numba
        # picks up the ATR-scaled TP without engine modification.
        # See backtest/strategies_v2.py::strategy_trend_follow_adx.
        try:
            from strategies_v2 import strategy_trend_follow_adx
        except ImportError:
            import os, sys as _sys
            _sys.path.insert(0, os.path.dirname(os.path.abspath(__file__)))
            from strategies_v2 import strategy_trend_follow_adx
        entry_long, entry_short, exit_long, exit_short = strategy_trend_follow_adx(
            ind, closes, highs, lows, params, n
        )

    elif strategy == "universal_long":
        # UNIVERSAL-STRATEGIES-PATCH v1.0
        # Universal LONG-only strategy with fully tunable parameters.
        # See backtest/strategies_universal.py::strategy_universal_long.
        try:
            from strategies_universal import strategy_universal_long
        except ImportError:
            import os, sys as _sys
            _sys.path.insert(0, os.path.dirname(os.path.abspath(__file__)))
            from strategies_universal import strategy_universal_long
        entry_long, entry_short, exit_long, exit_short = strategy_universal_long(
            ind, closes, highs, lows, params, n
        )
    elif strategy == "universal_short":
        # UNIVERSAL-STRATEGIES-PATCH v1.0
        # Universal SHORT-only strategy with fully tunable parameters.
        # See backtest/strategies_universal.py::strategy_universal_short.
        try:
            from strategies_universal import strategy_universal_short
        except ImportError:
            import os, sys as _sys
            _sys.path.insert(0, os.path.dirname(os.path.abspath(__file__)))
            from strategies_universal import strategy_universal_short
        entry_long, entry_short, exit_long, exit_short = strategy_universal_short(
            ind, closes, highs, lows, params, n
        )
    ...
    # Sortino (fix M5, M6, LA1: look-ahead bias)
    # FIX LA1: position[i] is set using closes[i], so using position[i]*returns[i]
    # gives "free" return on the entry bar. Shift position by 1 bar:
    # strategy_returns[i] = position[i-1] * returns[i]  (position from PREVIOUS bar earns THIS bar return)
    returns = np.diff(closes, prepend=closes[0]) / (np.concatenate([[closes[0]], closes[:-1]]) + 1e-10)
    position_lagged = np.zeros_like(position)
    position_lagged[1:] = position[:-1]  # shift: position[i-1]
    strategy_returns = position_lagged * returns
    downside = np.minimum(strategy_returns, 0)
    dstd = np.sqrt(np.mean(downside ** 2))
    bars_per_day = 39  # 10-min candles: 6.5h trading * 6 = 39 bars/day (was 78 for 5min)
