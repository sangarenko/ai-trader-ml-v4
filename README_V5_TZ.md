...
### ❌ КАТЕГОРИЧЕСКИ ЗАПРЕЩЕНО: ручная подкрутка порогов

**Лессон от пользователя (2026-08-18):**
> "Я не понимаю, почему ты эти пороги не выбрал эволюционно? почему ты сам их подкручиваешь?
> напиши это в тз, пороги определяет эволюция, тебе запрещено их трогать"

**Правило:** Все параметры моделей (thresholds, position sizes, hold times, commFilterMult) 
**ОПРЕДЕЛЯЮТСЯ ЭВОЛЮЦИЕЙ**, а не ручной настройкой. Запрещено:

1. ❌ Подкручивать `LONG_THRESHOLD` вручную (0.65 → 0.60 → 0.55)
2. ❌ Подкручивать `maxPositionCost` вручную (800 → 2000)
3. ❌ Подкручивать `positionSize` вручную (0.08 → 0.15)
4. ❌ Подкручивать `commFilterMult` вручную (0 → 1.5)
5. ❌ Подкручивать `MIN_HOLD_BARS`, `MAX_HOLD_BARS` вручную

**Единственный разрешённый путь:** генетический алгоритм ищет оптимальные параметры
для каждой версии (V1-V6) на основе OOS P&L после комиссии.

---

1. **НЕ использовать threshold=0.001** (равен комиссии)
2. **НЕ ставить commFilterMult вручную** — эволюция выберет
...
6. **НЕ подкручивать positionSize вручную** — эволюция выберет
7. **НЕ делать maxTradesPerHour вручную** — эволюция выберет
